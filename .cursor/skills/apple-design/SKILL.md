...
## Daylybread frontend mapping

This app is Ionic React + Capacitor, not a Motion/Framer Motion stack. Apply the principles above using these project tools:

- **Sheets and modals:** `IonModal` with breakpoints and swipe-to-dismiss (`BreadCrumbsModal`, `SongDetailSheet`, etc.). Prefer Ionic's gesture-driven sheet behavior over custom CSS transitions for draggable surfaces.
- **Drag and reorder:** `@dnd-kit` for sortable lists (home editor, worship flows). Keep 1:1 tracking and respect grab offset via dnd-kit sensors/transforms.
- **Carousels:** Swiper for momentum-based paging. Match projection and velocity handoff expectations from sections 5–6.
- **Haptics:** `@capacitor/haptics` for meaningful commit/snap moments (section 13). Fire haptics on the same frame as the visual state change.
- **Translucent chrome:** Use SCSS with `backdrop-filter` where Ionic variables are not enough (see `NavigationButtons.scss`).
- **Reduced motion:** Honor `prefers-reduced-motion` in component SCSS (see `PracticeMode.scss` for the existing pattern).
- **Buttons:** Prefer Ionic's `:active` / `fill="clear"` press states for instant pointer-down feedback on `IonButton`.

When adding new motion, default to critically damped (no bounce) unless the interaction is momentum-driven (flick, throw, sheet release).
